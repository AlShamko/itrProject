...
export const createSupportTicket = async (req: Request, res: Response) => {
    try {
        ...
        if (!summary || !priority) {
            return res
                .status(400)
                .json({error: "Summary and Priority are required"});
        }
        ...
        res
            .status(201)
            .json({message: "Support ticket created successfully", ticketId: filename});
    } catch (error) {
        ...
        res
            .status(500)
            .json({error: `Failed to create support ticket: ${errorMessage}`});
    }
};
